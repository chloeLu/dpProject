...
public class Playlist implements Iterable<StreamletInfo> {
	...
	public Playlist() {
		this.streamletInfoList = new ArrayList<StreamletInfo>();
		this.duration = "PT0S";
		this.minBufferTime = "PT0S";
		this.qualities = new ArrayList<QualitySpec>();
	}
	
	public void addStreamletSource(int index, int quality, String sourceURL) {
		StreamletInfo streamlet = null;
		
		if (index >= streamletInfoList.size()) {
			streamlet = new StreamletInfo();
			streamletInfoList.add(streamlet);
		} else {
			streamlet = streamletInfoList.get(index);
		}
		
		streamlet.setURLForQuality(quality, sourceURL);
	}
	...
	private List<StreamletInfo> streamletInfoList;
	...
	@Override
	public Iterator<StreamletInfo> iterator() {
		return this.streamletInfoList.iterator();
	}
}
